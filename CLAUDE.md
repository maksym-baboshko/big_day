...
> Cross-tool context: keep this file aligned with `AGENTS.md`.
> If a `GEMINI.md` file exists locally, keep that one in sync too.
...
├── features/
│   ├── countdown/
│   ├── game-session/                # auth, local session cache, shared game contracts
│   ├── language-switcher/
│   ├── theme-switcher/
│   └── wheel-of-fortune/
├── shared/
